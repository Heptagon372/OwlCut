...
t("DB 저장값 정리: 잘못된 색·범위·타입 걸러냄", () => {
  assert.deepEqual(
    layoutOptions({ photoOrder: [1, 0, 2, 3], slotSpacing: 3, slotRounding: 50, backgroundColor: "#FFD6E0", filterIntensity: 0.4 }),
    { photoOrder: [1, 0, 2, 3], slotSpacing: 3, slotRounding: 10, backgroundColor: "#FFD6E0", filterIntensity: 0.4 },
  );
  assert.deepEqual(layoutOptions({ backgroundColor: "red; drop table", photoOrder: "x" }), {
    photoOrder: null,
    slotSpacing: 0,
    slotRounding: 0,
